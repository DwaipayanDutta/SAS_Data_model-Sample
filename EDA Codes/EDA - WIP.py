...
class DataAnalyzer:
    ...
    def load_data(self):
        ...
        return df  # Assuming df is defined in the executed script
...
class EDA:
    ...
    def perform_kde_analysis(self):
        ...
        for var in continuous_vars:
            self.plotter.save_plot_to_ppt(lambda: sns.kdeplot(self.analyzer.df[var], fill=True), f'KDE Plot of {var}')
    ...
    def perform_outlier_analysis(self):
        ...
        for var in continuous_vars:
            self.plotter.save_plot_to_ppt(lambda: sns.boxplot(x='Outlier', y=var, data=self.analyzer.df), f'Box Plot of {var} Showing Outliers')

    def perform_bivariate_analysis(self):
        continuous_vars = self.analyzer.get_continuous_vars()
        
        for var in continuous_vars:
            self.plotter.save_plot_to_ppt(lambda: sns.kdeplot(data=self.analyzer.df, x=var, hue=self.analyzer.target_variable, fill=True), f'KDE Plot of {var} by {self.analyzer.target_variable}')
...
def main():
    ...
    eda.perform_kde_analysis()
    eda.perform_outlier_analysis()
    eda.perform_bivariate_analysis()
